# 👋 Hi, I'm Kuba

I'm a passionate **Full Stack Developer** focused on building modern, scalable web applications using the **MERN stack** — MongoDB, Express, React, Node.js 🚀  
I enjoy turning complex problems into simple, beautiful, and intuitive solutions 🎯

---

## 🛠️ Tech Stack

- 💻 **Frontend:** React ⚛️, HTML5 🧱, CSS3 🎨, JavaScript (ES6+) 📜  
- 🛠️ **Backend:** Node.js 🟢, Express.js  
- 🗃️ **Database:** MongoDB 🍃  
- 🔧 **Tools & Other:** Git 🔀, REST APIs 🌐, npm 📦, Postman 📫

---

## 🚧 Currently Working On

- 🚀 Building full-stack web apps with authentication & real-time features  
- 📚 Learning TypeScript & Next.js  
- ☁️ Exploring cloud deployment: Vercel, Render, Heroku, Docker

---

## 📁 Featured Projects

Here are some of the projects I'm proud of:

- 🎰 **LottoApp** — Full-stack EuroJackpot number generator   
  🌐 (https://lotto-j6xo.onrender.com/)

- ❤️ **Amorio** — React + MongoDB dating site with live messaging 💬, photo uploads 📷, and geolocation 📍  
  🌐 (https://amorio-g8q6.onrender.com/)

---

## 📫 Get in Touch

- 🌐 My site: [JakubSulisz](https://jakub728.github.io/my-site/)
- 💼 GitHub: [github.com/jakub728](https://github.com/jakub728)  
- 📧 Email: sulisz.jakub@gmail.com  
- 💬 LinkedIn: [linkedin.com/jakub-sulisz](https://www.linkedin.com/in/jakub-sulisz-5a1660368)

---

## ⚡ Fun Fact

> I love clean code, good coffee ☕, and solving tough bugs at 2am 🐞💡

Thanks for stopping by! Let’s build something amazing together 💻✨
